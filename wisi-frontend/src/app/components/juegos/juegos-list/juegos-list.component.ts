...
@Component({
  ...
  template: `
    <div class="juegos-container">
        ...
        <table class="table table-striped table-hover">
          ...
          <tbody>
            <tr *ngFor="let juego of juegos; let i = index">
              <td>{{ i + 1 }}</td>
              <td>{{ juego.nombre }}</td>
              <td>{{ juego.Sala?.nombre || 'Sin asignar' }}</td>
              <td>
                <button 
                  class="btn btn-info btn-sm me-1" 
                  [class.disabled]="!canEdit()"
                  [disabled]="!canEdit()"
                  (click)="canEdit() ? editJuego(juego) : null">
                  Editar
                </button>
                <button 
                  ...
                  [disabled]="!canDelete()"
                  (click)="canDelete() ? deleteJuego(juego.id) : null">
                  Eliminar
                </button>
              </td>
            </tr>
      ...
      <div *ngIf="juegos.length === 0" class="no-data">
      ...
      <div *ngIf="showSalaModal" class="modal-overlay" (click)="closeSalaSelector()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h3>{{ selectedJuego ? 'Editar Juego' : 'Crear Nuevo Juego' }}</h3>
            <button class="close-btn" (click)="closeSalaSelector()">&times;</button>
          </div>
          <div class="modal-body">
            <form (ngSubmit)="createJuego()" #juegoForm="ngForm">
              ...
              <div class="form-group" *ngIf="!selectedJuego">
                <label for="salaSelect">Sala:</label>
                <select 
                  id="salaSelect" 
                  name="salaSelect"
                  [(ngModel)]="nuevoJuego.sala_id"
                  class="form-control"
                  required
                >
              ...
              <div class="form-actions">
                ...
                <button type="submit" class="btn btn-success" [disabled]="!juegoForm.form.valid">
                  {{ selectedJuego ? 'Actualizar Juego' : 'Guardar Juego' }}
                </button>
              </div>
            </form>
            
            <div *ngIf="userSalas.length === 0 && !selectedJuego" class="no-salas">
...
export class JuegosListComponent implements OnInit, OnDestroy {
  juegos: any[] = [];
  userSalas: any[] = [];
  ...
  editJuego(juego: any): void {
    this.selectedJuego = juego;
    this.nuevoJuego.nombre = juego.nombre;
    this.nuevoJuego.sala_id = juego.sala_id;
    this.showSalaModal = true;
    this.loadUserSalas();
  }
